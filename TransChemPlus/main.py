...
def atom_features(atom: Chem.rdchem.Atom):
    ...
    return [
        atomic_num, formal_charge, total_h, implicit_valence, explicit_valence,
...
def atom_features0(atom: Chem.rdchem.Atom):
    return [
        atom.GetAtomicNum(),
...
csv_path = "/home/devuser/cgy/chemprop-Mob/data/O2_Permeability_Barrer.csv"
smiles_list, targets, _= load_smiles_and_targets(csv_path, smiles_column="smiles", target_column="O2_Permeability_Barrer")
...
model = SimpleGNN(in_dim, hidden_channels=128, num_layers=6, return_embedding=False).to(device)
...
best_val = float("inf")
best_state = None
save_path = "/home/devuser/cgy/GCN/GCN/models/best_gnn_model_finetune.pth"
...
for epoch in range(1, num_epochs + 1):
    ...
    val_mse, val_preds, val_ys = evaluate(model, val_loader, device)
...
# ---------- Load Best Model ----------
embedding_model = SimpleGNN(in_dim, hidden_channels=128, num_layers=6, return_embedding=False).to(device)
...
train_mse, train_preds, train_ys = evaluate(embedding_model, train_loader, device)
test_mse, test_preds, test_ys = evaluate(embedding_model, test_loader, device)

def to_numpy(x):
    return x if isinstance(x, np.ndarray) else x.detach().cpu().numpy()

train_preds = to_numpy(train_preds)
train_ys = to_numpy(train_ys)

test_preds = to_numpy(test_preds)
test_ys = to_numpy(test_ys)


train_r2 = r2_score(train_ys, train_preds)
test_r2 = r2_score(test_ys, test_preds)
...
save_pred_csv = "/home/devuser/cgy/GCN/GCN/all_predictions.csv"

rows = []

# Train
for smi, y_true, y_pred in zip(train_smi, train_ys, train_preds):
    rows.append({
        "smiles": smi,
        "y_true": float(y_true),
        "y_pred": float(y_pred),
...
# Test
for smi, y_true, y_pred in zip(test_smi, test_ys, test_preds):
    rows.append({
        "smiles": smi,
        "y_true": float(y_true),
        "y_pred": float(y_pred),
