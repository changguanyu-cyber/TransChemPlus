...
def load_smiles_and_targets(
    ...
    df = df.dropna(subset=[smiles_column, target_column]).reset_index(drop=False)
    csv_ids = df.index.tolist()
    ...
    # ---------- Filtering ----------
    for idx, (smi, tgt) in zip(df["index"], zip(df[smiles_column], df[target_column])):
        if "[R" in smi:
            continue
...
class SMILESDataset(Dataset):
    def __init__(self, smiles_list, targets=None, ids_list=None):
        self.nan_count = 0
        self.mol_ids = ids_list
    ...
    def __getitem__(self, idx):
        ...
        data = smiles_to_data(self.smiles[idx], y)
        if self.mol_ids is not None:
            data.mol_id = torch.tensor(self.mol_ids[idx], dtype=torch.long)
        return data
...
smiles_list, targets, _, ids_list= load_smiles_and_targets(csv_path, smiles_column="PSMILES", target_column="Atomization_Energy_eV")
train_smi, test_smi, train_y, test_y, train_id, test_id = train_test_split(
    smiles_list, targets, ids_list, test_size=0.2, random_state=42)

train_smi, val_smi, train_y, val_y, train_id, val_id = train_test_split(
    train_smi, train_y, train_id, test_size=0.1, random_state=42)
Data_loader = DataLoader(SMILESDataset(smiles_list, targets, ids_list), batch_size=64, shuffle=False)
train_loader = DataLoader(SMILESDataset(train_smi, train_y, train_id), batch_size=64, shuffle=False)
val_loader   = DataLoader(SMILESDataset(val_smi, val_y, val_id), batch_size=64)
test_loader  = DataLoader(SMILESDataset(test_smi, test_y, test_id), batch_size=64)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
in_dim = len(atom_features(Chem.MolFromSmiles("C").GetAtomWithIdx(0)))

# ---------- Load Best Model ----------
model = SimpleGNN(in_dim, hidden_channels=128, num_layers=6, return_embedding=True).to(device)
model.load_state_dict(torch.load("/root/autodl-tmp/GCN/best_gnn_model.pth"))
model.eval()
all_sequences = []
all_ids = []

with torch.no_grad():
    for batch in Data_loader:   # or full_loader
        batch = batch.to(device)

        seq_output = model(batch.x, batch.batch,
                          batch.edge_index_bond,
                          batch.edge_index_virtual_intra,
                          batch.edge_index_virtual_conn)

        for i, out in enumerate(seq_output):
            mol_id = batch.mol_id[i]   
            all_sequences.append(out.cpu().numpy())
            all_ids.append(mol_id)
import pickle

with open("/root/autodl-tmp/GCN/gnn_sequences.pkl", "wb") as f:
    pickle.dump({"ids": all_ids, "sequences": all_sequences}, f)
print('right')
